...
async def on_member_join(member):
    channel = discord.utils.get(member.guild.channels, name='general chat')
    await channel.send(
...
async def credits(ctx):
    ...
    await ctx.send(
        'Thanks to `KingSticky` for helping with the `?die` and `?creditz` command'
    )
...
@client.command(name='remove',
                help='This command removes an item from the list')
...
@client.command(name='leave',
                help='This command stops makes the bot leave the voice channel'
                )
async def leave(ctx):
...
client.run('ODY3MjUzNzIyODM2NTAwNTEx.YPea-A.vFWldHy8lzO0VivCsCqQfm10QKU')
